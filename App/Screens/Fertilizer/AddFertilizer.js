...
class AddFertilizer extends Component {
    ...
    addFertilizer = () => {
        ...
        if (name.trim().length >= 3 && price.trim().length >= 1 && application.trim().length >= 10 && imageSource.uri) {
            ...
            this.props.addFertilizer(formdata, this.props.token);
        } else {
            if (name.trim().length == 0) {
                ToastAndroid.show('fettilizer name is required', ToastAndroid.SHORT);
                return;
            } else if (price.trim().length == 0) {
                ToastAndroid.show('fertilizer price is required', ToastAndroid.SHORT);
                return;
            } else if (application.trim().length == 0) {
                ToastAndroid.show('fertilizer application is required', ToastAndroid.SHORT);
                return;
        ...
        return (
            <ScrollView contentContainerStyle={{ height: height - 80, width }} keyboardDismissMode="interactive" keyboardShouldPersistTaps="handled" >
                <CompanyPanel navigation={this.props.navigation}
                    ...
                    <View style={{ flex: 0.6, padding: width * 1 / 20 }}>
                        <View style={{ flex: 1 / 4 }}>
                            <Item style={{ flex: 1 }} >
                                ...
                                <Input style={{ flex: 0.9, color: '#fff' }} placeholderTextColor={"#fff"} placeholder="Fertilizer Name" onChangeText={(name) => this.setState({ name })} />
                            </Item>
                    ...
                    <View style={{ flex: 0.2 }}>
                        {this.props.isProgress ? <Spinner /> : <Button onPress={this.addFertilizer} style={{ alignSelf: "center", width: width * 0.75, justifyContent: "center", borderRadius: width / 12, backgroundColor: "#fafafa" }} ><Text style={{ alignSelf: "center", color: "#272727" }} >Submit</Text></Button>}
                    </View>
                </CompanyPanel>
            </ScrollView>
        )
...
const mapDispatchToProps = dispatch => {
    return {
        addFertilizer: (obj, token) => dispatch(AppAction.addFertilizer(obj, token)),
        clearFertilizers: () => dispatch(AppAction.clearFertilizers())
    };
};
